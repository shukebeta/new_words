...
void main() {
  group('AuthAwareProvider', () {
    ...
    test('should call onLogin when auth state changes to authenticated', () async {
    ...
    test('should call onLogin every time auth state changes to true (prevents data leakage)', () async {
      await provider.onAuthStateChanged(true);
      provider.resetForTesting(); // Reset for testing
      
      await provider.onAuthStateChanged(true);
      
      expect(provider.onLoginCalled, true); // Should be called again to ensure clean state
    });
    
    test('should call onLogout when auth state changes to unauthenticated', () async {
      // First authenticate
      await provider.onAuthStateChanged(true);
    ...
    test('should clear data on every login to prevent user data leakage', () async {
    ...
    test('should call onLogin when auth state changes to true (clearAllData called separately by AppStateProvider)', () async {
      // Add some data first
    ...
    test('should simulate complete AppStateProvider workflow for data isolation', () async {
